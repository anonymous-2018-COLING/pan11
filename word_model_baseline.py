...
class MTANet(object):
    def __init__(self, is_training=True, config=Config()):
        ...
        self.word_dropout_kp = tf.placeholder(tf.float32, name='word_keep_prob')
    ...
    def _word_model(self):
        ...
        pooled_outputs = []
        with tf.name_scope("conv-maxpool-%s" % 3):
            # Convolution Layer
            filter_shape = [3, self.config.word2vec_D, 1, self.config.num_filters]
            #b = tf.Variable(tf.constant(0.1, shape=[self.config.num_filters]), name="b")
            conv_filter = tf.Variable(tf.truncated_normal(filter_shape, stddev=0.1), name="conv_filter")
            conv_output = tf.nn.conv2d(
                input=conv_input,
                filter=conv_filter,
                strides=[1, 1, self.config.word2vec_D, 1],# ??????????
                padding="SAME",
                name="conv")
            # Apply nonlinearity
            #h = tf.nn.relu(tf.nn.bias_add(conv_output, b), name="relu")
            h = tf.nn.dropout(conv_output, self.word_dropout_kp)
            # Maxpooling over the outputs
            pooled = tf.nn.max_pool(
                value=h,
                ksize=[1, self.config.word_len_max, 1, 1],
                strides=[1, 1, 1, 1],
                padding='VALID',
                name="pool")
        ...
        #print(h_pool)
        h_pool_flat = tf.reshape(pooled, [-1, self.config.num_filters])
        print(h_pool_flat)
        return h_pool_flat
    ...
    def _fc(self, emb):
        ...
        with tf.name_scope('fc1'):
            f1 = tf.layers.dense(inputs=emb, units=5*72, activation=tf.nn.tanh)
        f1_dropout = tf.nn.dropout(f1, self.dropout_keep_prob)
        with tf.name_scope('fc2'):
            fc2 = tf.layers.dense(inputs=f1_dropout, units=72, activation=tf.nn.relu)
        return fc2

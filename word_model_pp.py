...
class MTANet(object):
    def __init__(self, is_training=True, config=Config()):
        ...
        self.Y2 = tf.placeholder(tf.int32, shape=[None, 2], name='Y2')
    ...
    def loss(self, emb):
        ...
        if self.config.classifier:
            ...
            classifier_tup = [('gender', 19320, emb[0]), ('age', 2, emb[1])]
            for step, item in enumerate(classifier_tup):
                # get one_hot
                item_labels = tf.one_hot(
                    indices=self.Y[:, step],
                    depth=classifier_tup[step][1]
                )
                item_logits = classifier_tup[step][2]

                # loss
                item_loss = tf.reduce_mean(
                    tf.nn.softmax_cross_entropy_with_logits(
                        logits=item_logits,
                        labels=item_labels
                    )
                )
                self.loss_list.append(item_loss)
                tf.summary.scalar(classifier_tup[step][0] + ' loss:\t', item_loss)

                # acc
                item_acc = tf.reduce_mean(
                    tf.cast(
                        tf.equal(
                            # axis, Describes which axis of the input Tensor to reduce across.
                            tf.argmax(input=item_logits, axis=1),
                            tf.argmax(input=item_labels, axis=1)),
                        tf.float32))
                self.acc_list.append(item_acc)
                tf.summary.scalar(classifier_tup[step][0] + ' acc:\t', item_acc)
            ...
            # total_loss
            self.total_loss = sum(self.loss_list) #+ reg
    ...
    def _fc(self, emb):
        ...
        if not self.config.classifier:
            ...
            with tf.name_scope('fc_gender'):
                fc_gender = tf.layers.dense(inputs=emb_dropout, units=2)
            with tf.name_scope('fc_age'):
                fc_age = tf.layers.dense(inputs=emb_dropout, units=5)
            return fc_gender, fc_age
